    ...
    <section id="contact" className="py-16 md:py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-10 md:mb-16">
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-900 text-white text-xs font-bold uppercase tracking-widest mb-4 animate-fade-in-up">
            <Sparkles size={14} className="text-yellow-400" />
            ¿Hablamos?
          </span>
          <h2 className="text-3xl md:text-6xl font-extrabold text-slate-900 tracking-tight mb-3 md:mb-4">
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-20">
          ...
          {/* Right Column: Gamified Form (ORDER 1 on Mobile, ORDER 2 on Desktop) */}
          <div className="lg:col-span-7 order-1 lg:order-2">
            <div className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden">
               {/* Form Background Blob */}
               <div className="absolute top-0 right-0 w-64 h-64 bg-brand-50 rounded-full blur-3xl -mr-20 -mt-20"></div>

               <h3 className="text-xl md:text-2xl font-bold text-slate-900 mb-6 md:mb-8 relative z-10 flex items-center gap-2">
                  Envíanos un mensaje
                  <span className="text-brand-500 animate-bounce">👇</span>
               </h3>
               ...
               <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8 relative z-10">
                 ...
                 <div className="grid md:grid-cols-2 gap-4 md:gap-6">
                   <div className="space-y-2">
                     ...
                     <input 
                        ...
                        className={`w-full px-5 py-3 md:py-4 bg-slate-50 border-2 rounded-2xl outline-none transition-all duration-300 font-medium ${focusedInput === 'name' ? 'border-brand-500 bg-white ring-4 ring-brand-500/10 scale-[1.02]' : 'border-transparent hover:border-slate-200'}`}
                        required
                     />
                   </div>
                   <div className="space-y-2">
                     ...
                     <input 
                        ...
                        onBlur={() => setFocusedInput(null)}
                        className={`w-full px-5 py-3 md:py-4 bg-slate-50 border-2 rounded-2xl outline-none transition-all duration-300 font-medium ${focusedInput === 'email' ? 'border-brand-500 bg-white ring-4 ring-brand-500/10 scale-[1.02]' : 'border-transparent hover:border-slate-200'}`}
                        required
                     />
                   </div>
                 </div>
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                     {interests.map((item) => (
                       <button
                         ...
                         className={`relative p-3 md:p-4 rounded-2xl border-2 text-left transition-all duration-300 flex items-center gap-3 ${
                 ...
                 <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      onBlur={() => setFocusedInput(null)}
                      className={`w-full px-5 py-3 md:py-4 bg-slate-50 border-2 rounded-2xl outline-none transition-all duration-300 font-medium resize-none ${focusedInput === 'message' ? 'border-brand-500 bg-white ring-4 ring-brand-500/10 scale-[1.02]' : 'border-transparent hover:border-slate-200'}`}
                      required
                    ></textarea>
                 </div>
                 ...
                 <button 
                   type="submit"
                   className="w-full py-4 md:py-5 bg-slate-900 text-white font-bold rounded-2xl shadow-xl shadow-slate-900/20 hover:shadow-2xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 group"
                 >
          ...
          {/* Left Column: Direct Action & Socials (ORDER 2 on Mobile, ORDER 1 on Desktop) */}
          <div className="lg:col-span-5 space-y-6 md:space-y-8 flex flex-col order-2 lg:order-1 mt-4 lg:mt-0">
            ...
            <a 
              ...
              className="group relative w-full p-6 bg-[#25D366] rounded-[2rem] shadow-[0_20px_50px_-12px_rgba(37,211,102,0.5)] hover:shadow-[0_20px_60px_-12px_rgba(37,211,102,0.7)] transition-all duration-300 transform hover:-translate-y-2 overflow-hidden flex items-center justify-between"
            >
               {/* Pulse Effect */}
               ...
               <div className="bg-white/20 p-2 rounded-full transform group-hover:rotate-45 transition-transform duration-500 hidden sm:block">
                 <ArrowRight className="text-white" size={24} />
               </div>
            </a>

            <div className="bg-slate-50 p-6 md:p-8 rounded-[2rem] border border-slate-100 flex items-center gap-5 group hover:border-brand-200 transition-colors">
              ...
              <div className="overflow-hidden">
                ...
                <a href="mailto:gaorsystem@gmail.com" className="text-lg md:text-xl font-bold text-slate-900 hover:text-brand-600 transition-colors block truncate">
